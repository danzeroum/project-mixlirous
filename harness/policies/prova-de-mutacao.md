...
## Sobre os números que estavam aqui

Esta política dizia "as 118" em dois lugares, e o número real já era **163** quando o CP-041 foi
escrever a extração. Ninguém mentiu: as asserções cresceram e a prosa não. É a razão pela qual
nenhuma contagem é restatada aqui agora — **o fiscal imprime quantas provou**, e quem quiser o
número lê o laudo. Número copiado deriva, e este derivou.
...
não alcança — as asserções `file_lacks` com regex expressiva, onde gerar um texto que case
exige entender a **intenção** da regra e não só a sua forma.

Escrever uma por asserção à mão seria a lista duplicada que o §12 proíbe na mesma frase em que
pede a declaração: ela derivaria da asserção real no primeiro dia em que alguém mudasse um `pattern` e
esquecesse o bloco.
...
A prova copia o repositório e roda o fiscal de conformidade uma vez por asserção bloqueante:
cerca de um minuto. O hook
